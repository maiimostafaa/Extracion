...
import CafeFinderCard from "../assets/components/CafeFinderCard";
...
export default function CafeFinderScreen() {
  ...
  const handleRecenterMap = useCallback(async () => {
    try {
      let { status } = await Location.requestForegroundPermissionsAsync();
      if (status !== "granted") {
        Alert.alert("Permission Required", "Location permission is needed to center the map on your location.");
        return;
      }
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        const distanceToExpanded = Math.abs(currentPosition - MODAL_TOP_POSITION);
        const distanceToCollapsed = Math.abs(currentPosition - MODAL_COLLAPSED_POSITION);
        const distanceToMinimized = Math.abs(currentPosition - MODAL_MINIMIZED_POSITION);
        ...
        if (Math.abs(gestureState.dy) > threshold) {
          if (gestureState.dy < 0) {
            ...
            if (currentPosition <= (MODAL_TOP_POSITION + MODAL_COLLAPSED_POSITION) / 2) {
              targetValue = MODAL_TOP_POSITION;
              newState = 'expanded';
            } else {
              targetValue = MODAL_COLLAPSED_POSITION;
            ...
            if (currentPosition >= (MODAL_COLLAPSED_POSITION + MODAL_MINIMIZED_POSITION) / 2) {
          ...
          const minDistance = Math.min(distanceToExpanded, distanceToCollapsed, distanceToMinimized);
