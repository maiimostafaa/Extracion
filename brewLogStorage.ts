...
export const deleteBrewLog = async (id: number) => {
    ...
    if (entryToDelete && entryToDelete.image && !entryToDelete.image.startsWith('http')) {
        try {
...
export const storeImagePermanently = async (sourceUri: string, brewLogId: number): Promise<string> => {
    try {
